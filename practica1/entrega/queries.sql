...
--#&&
SELECT a.name, (m.price / p.square_feet) AS price_m2, COUNT(r.id_apartment) AS reviews
FROM apartment AS a,
     money AS m,
     reviews AS r,
     properties AS p,
...
--#&&
SELECT a.street, COUNT(a.listing_url) AS num, AVG(m.price) AS price
FROM apartment AS a,
     money AS m
...
--#&&
SELECT h.host_name,
       SUM((1 / m.price) * (1 + (hi.host_is_superhost::int)) * contado * hi.host_listings_count) AS score
FROM host AS h,
     apartment AS a,
     money AS m,
     host_info AS hi,
     host_verifications AS hv,
     (SELECT COUNT(hv2.id_host) AS contado FROM host_verifications AS hv2 where hv2.id_host = hv.id_host) as s
WHERE h.host_url = a.id_host
  AND a.id_host = hi.id_host
  AND a.listing_url = m.listing_url
  AND hv.id_host = h.host_url
  AND m.price != 0
